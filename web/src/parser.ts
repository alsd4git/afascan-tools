...
export function extractRecord(sourceFile: string, text: string): AfaScanRecord {
  ...
  return {
    ...
    basal_metabolic_rate_kcal: integerValue(first([/Basal\s*Metabolic\s*Rate\s*[:=]?\s*(\d{3,4})\b/im], text)),
    ...
    water_percent: numberValue(first([/Water\s*Percent\s*\)?\s*[:=]?\s*([0-9]+[.,][0-9])%/im], text)),
    segment_fat_kg: null,
